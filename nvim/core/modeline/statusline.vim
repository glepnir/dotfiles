...
function! SetStatusline()
    set statusline=
    set statusline+=%#HomeMode#%{VimacslineMode()}
    set statusline+=%#Seperate1#
    set statusline+=\
    set statusline+=\ 
    ...
    if !empty(get(g:,'coc_git_status',''))
        ...
        set statusline+=\ 
        set statusline+=%#Seperate5#
        set statusline+=\
    endif
    if !empty(expand('%:t')) && empty(get(g:,'coc_git_status',''))
        set statusline+=%#emptySeperate1#
        set statusline+=\
    endif
    if empty(Filesize())
        set statusline+=%#emptySeperate1#
    ...
    set statusline+=%#CocBar#
    set statusline+=\ 
    set statusline+=%{CocStatusBar()}
    set statusline+=%=
    set statusline+=%#Seperate4#
    set statusline+=\
    set statusline+=\ 
...
augroup statusline
	autocmd!
	autocmd FileType,WinEnter,BufWinEnter * call SetStatusline()
