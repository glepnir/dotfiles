...
function! VimacsLineinfo() abort
  ...
  \      &filetype ==? 'defx'             ? ' ' :
  \      &filetype ==? 'denite'           ? ' ' : \      &filetype ==? 'tagbar'           ? ' ' : \      &filetype ==? 'vista_kind'       ? ' ' : \      &filetype ==? 'vista'            ? ' ' :
...
set statusline=
set statusline+=%1*%{VimacslineMode()}
set statusline+=%2*%{VimacsFilenameActive()}
set statusline+=%3*%{VimacsLineCocError()}
set statusline+=%4*%{VimacsLineCocWarn()}
set statusline+=%5*%{VimacsLineGit()}
set statusline+=%6*%{CocStatusBar()}
set statusline+=%7*%{VimacsLineFileformat()}
set statusline+=%8*%{FileEncoding()}
